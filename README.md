...
- `Makefile` — ビルド／インストール
- `karabiner/curpop.json` — Karabiner-Elements の Complex Modifications 用設定
- `LICENSE` — MIT
...
`PREFIX` でインストール先を変えられる（例: `make install-all PREFIX=/usr/local`）。`install-karabiner` は配置時にバイナリパスを `$(PREFIX)/bin/curpop` に書き換えるので、Karabiner 側の `shell_command` も追従する。

インストール後、Karabiner-Elements を開き **Settings → Complex Modifications → Add predefined rule** から *curpop* を有効化する。
...
別キーに割り当てたい場合は `karabiner/curpop.json` の `from.key_code` を編集する。修飾キー機能を犠牲にしたくないなら `f13`〜`f19` などの未使用キーが扱いやすい。
